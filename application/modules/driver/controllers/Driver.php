...
class Driver extends MX_Controller {
	...
	public function driverMaster()
	{
		...
		$column = "isactive";
		$value = "1";
		$data['sitelist'] = $this->driver_model->getData($select, $tableName, $column, $value);
	...
	echo json_encode($response);
 	...
 	public function driverList(){
 		$driver_table =  DRIVER_TABLE;
 		$filds = "driver_id,driver_fname,driver_mname,driver_lname,driver_add,driver_photo,driver_bdate,driver_mobno,driver_mobno1,site_id,ledger_id";
 	...
 	public function attnReport(){
 		...
 		$driver = $this->helper_model->selectQuery("SELECT driver_id,user_check_in,day_type,DATE_FORMAT(user_check_in, '%Y-%m-%d') as date from driver_attendance where user_check_in >= '$from_date' and user_check_in <= '$to_date' and driver_id = '$driver_id'");
		...
		{ 
			...
			if($key !== false){
				...
				else
				{
					$data .= '<tr>
						<td>'.$from_date.'</td>
						<td>Present at '.$driver[$key]['user_check_in'].'(Half Day)</td>
					</tr>';
				$j++;
				}
			}
