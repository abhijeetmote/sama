...
/*		$select = 'site_id,site_name';
 	...
 	public function labourAttend(){
 		...
 		$date = date('Y-m-d');
        $data['labourdetails'] = $this->helper_model->selectQuery("SELECT labour_master.labour_id,labour_fname,labour_lname FROM labour_master where labour_master.labour_id not in (select labour_id from labour_attendance where DATE_FORMAT(user_check_in, '%Y-%m-%d') = '$date')");
